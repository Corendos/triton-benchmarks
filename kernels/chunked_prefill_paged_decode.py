...
def chunked_prefill_paged_decode(
    ...
    if SHOULD_LOG:
        print("Calling kernel_paged_attention_2d with grid: {}".format(grid))
        print(f"num_query_heads: {num_query_heads}")
        print(f"num_queries_per_kv: {num_queries_per_kv}")
        print(f"num_queries_per_kv_padded: {num_queries_per_kv_padded}")
        print(f"block_table_stride: {block_table.stride(0)}")
        print(f"query_stride_0: {query.stride(0)}")
        print(f"query_stride_1: {query.stride(1)}")
        print(f"output_stride_0: {output.stride(0)}")
        print(f"output_stride_1: {output.stride(1)}")
        print(f"x: {key_cache.shape[4]}")
        print(f"stride_k_cache_0: {key_cache.stride(0)}")
        print(f"stride_k_cache_1: {key_cache.stride(1)}")
        print(f"stride_k_cache_2: {key_cache.stride(2)}")
        print(f"stride_k_cache_3: {key_cache.stride(3)}")
        print(f"stride_k_cache_4: {key_cache.stride(4)}")
        print(f"stride_v_cache_0: {value_cache.stride(0)}")
    ...
    if SHOULD_LOG:
        ast_src: triton.compiler.compiler.ASTSource = compiled_kernel.src
        print(f"kernel_paged_attention_2d: {ast_src.constants}")
        print(f"kernel_paged_attention_2d: {compiled_kernel.asm['ttir']}")
        #print(f"kernel_paged_attention_2d: {compiled_kernel.asm['ttgir']}")
        #print(f"kernel_paged_attention_2d: {compiled_kernel.asm['llir']}")
